import { CalendarDays, CreditCard, HeartHandshake, MessageSquareText, UsersRound, Sparkles } from 'lucide-react';
    ...
    <div className="admin-page">
      ...
      <section className="portal-overview-hero">
        <article className="portal-overview-primary">
          ...
          <div>
            <h2>{parent.nameEn}<span lang="ar" dir="rtl"> · {parent.nameAr}</span></h2>
            <p><BilingualText value={bi('A calm family view with no invented invoices, schedules or contact details.', 'واجهة أسرية هادئة دون فواتير أو جداول أو بيانات اتصال مختلقة.')} /></p>
          </div>
        </article>
        <article className="portal-overview-score">
          <small><BilingualText value={bi('Linked Children', 'الأبناء المرتبطون')} /></small>
          ...
          <BilingualText value={bi(`${attended}/${scheduled} preview attendance`, `${attended}/${scheduled} حضور تجريبي`)} />
        </article>
      </section>
      ...
      <section className="portal-action-grid">
        <Link to="/parent/children">
          ...
          <small><BilingualText value={bi('Review linked player profiles', 'مراجعة ملفات اللاعبين المرتبطين')} /></small>
        </Link>
        <Link to="/parent/schedule">
          <CalendarDays />
          <BilingualText value={bi('Family Schedule', 'جدول الأسرة')} />
          <small><BilingualText value={bi('Preview training context', 'معاينة سياق التدريب')} /></small>
        </Link>
        <Link to="/parent/messages">
          <MessageSquareText />
          <BilingualText value={bi('Messages', 'الرسائل')} />
          <small><BilingualText value={bi('Communication-ready empty state', 'حالة فارغة جاهزة للتواصل')} /></small>
        </Link>
      </section>

      <section className="portal-overview-list">
        <header>
          <BilingualText value={bi('Children Snapshot', 'لمحة عن الأبناء')} />
          <Sparkles />
        </header>
        <div>
          {children.map(player => player && (
            <article key={player.id}>
              <strong>{player.nameEn}<span lang="ar" dir="rtl"> · {player.nameAr}</span></strong>
              <small><BilingualText value={getSport(player.sportId)?.name ?? bi('Sport Preview', 'معاينة الرياضة')} /></small>
            </article>
          ))}
        </div>
        <Link className="admin-link-button" to="/parent/payments">
          <CreditCard />
          <BilingualText value={bi('Payments Preview', 'معاينة المدفوعات')} />
        </Link>
      </section>
    </div>
  );
